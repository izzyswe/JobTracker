...
class Application:
    checkSys = lambda : os.system('cls') if os.name == 'nt' else os.system('clear')
    HEADER: tuple = ("id", "Company Name", "Position", "Address", "CV or Resume", "Web Link", "Status", "Date Applied", "Deadline")
    APPLICATION_STATUS: tuple = ("applying", "applied", "not applied yet")
    STATUS_INDICATORS: tuple = ({
...
class Worksheet:
    ...
    def generateTitles(worksheet):
        ...
        for i, n in zip(a.upperCol, a.colName):
            cell = i + "2"
            worksheet[cell] = n

        # after filling row 1 with your headers
        for col_idx, header in enumerate(a.colName, start=1):
    ...
    def checkWorksheet(self, filename="Job-Tracker.xlsx") -> str:
        if os.path.exists(filename):
            wb = load_workbook(filename)
        else:
            wb = Workbook()
            ws = wb.active
            self.generateTitles(ws)
        return wb
...
class WorksheetActions:
    ...
    def getJobDetails(worksheet):
        ...
        for i in a.colName:
            ...
            elif i == a.colName[7] or i == a.colName[8]:
                while True:
        ...
        if hasAppliedStatus:
            ...
            cellFill = f"{get_column_letter(a.colName.index("Status") + 1)}{worksheet.max_row}"
...
class MainMenu:
    def Menu(worksheet):
        ...
        while True:
            ...
            match opts:
                case "1":
                    wA.getJobDetails(worksheet)
                case "2":
                    wA.searchJobDetail(worksheet)
                case "3":
                    wA.updateJobDetail()
                case "4":
